...
exports.pathRewriteMiddleware = function(i18n) {
	...
	return function locPathRewrite(req, res, next) {
...
Translation.prototype.applyLocaleStrategy = function(req, res) {
	...
	if (req && req.accepts('html') && this.i18n.getLocaleFrom) {
		...
		for (var i = 0; i < from.length; i++) {
			strategy = this.getStrategy(from[i]);
		...
		if (locale) {
			...
			for (var i = 0; i < to.length; i++) {
				strategy = this.getStrategy(to[i]);
...
Translation.prototype.getStrategy = function(strategy) {
	...
	try {
		loaded = utils.getDependency(this.i18n.strategies, strategy);
